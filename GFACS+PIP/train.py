...
def train_instance(
    ...
    print(">> Training...")
    print("is_train_dual_decoder", is_train_dual_decoder)
    print("generate_simulated_mask", generate_simulated_mask)
    print("dual_decoder is not None:", dual_decoder is not None)
    print("model param:", model.state_dict()["emb_net.v_lin0.weight"][0] )
    print("model training:", model.training)
    print("lazy model param:", lazy_model.state_dict()["emb_net.v_lin0.weight"][0] )
    print("lazy model training:", lazy_model.training)
    print("dual_decoder param:", dual_decoder.state_dict()['lins.0.weight'][0, :5] )
    print("dual_decoder training:", dual_decoder.training)

    if isinstance(optimizer, list):
        optimizer_net, optimizer_dual_decoder = optimizer
        optimizer_net.zero_grad()
        if is_train_dual_decoder:
            optimizer_dual_decoder.zero_grad()
    else:
        optimizer.zero_grad()
    ...
    for pyg_data, tw, distances, positions in data:
        ...
        if dual_decoder is not None and is_train_dual_decoder:
        ...
        if not is_train_dual_decoder and dual_decoder is not None:
            # print(">> use embedding from lazy model...")
            with torch.no_grad():
                _, embedding = lazy_model(pyg_data.clone(), tw_normalize, return_embedding=True)
        ...
        aco = ACO(
            ...
            sl_mask_type = args.dual_decoder_type
        )
        
        if guided_exploration:
            costs_nls, log_probs, paths_nls, costs_raw, paths, timeout_nls, timeout_nodes_nls, timeout_raw, timeout_nodes_raw, out = aco.sample_local_search(invtemp, return_loss = is_train_dual_decoder, return_auc=generate_simulated_mask and dual_decoder is not None)
            if use_penalty:
        ...
        tb_loss = tb_loss / batch_size
        tb_loss_nls = tb_loss_nls / batch_size if guided_exploration else torch.tensor(0.0, device=DEVICE)
        sl_loss = sl_loss / batch_size if is_train_dual_decoder else torch.tensor(0.0, device=DEVICE)
        if args.balance_sl_weight:
            factor = 1. if sl_loss >= 1. else 0.
            loss = tb_loss + tb_loss_nls + sl_loss * 10 ** (torch.floor(torch.log10(tb_loss.detach())) - factor )
        else:
            loss = tb_loss + tb_loss_nls + sl_loss
        loss.backward()
    ...
    # sum_loss_sl = sum_loss_sl /count  if is_train_dual_decoder else torch.tensor(0.0, device=DEVICE)
    # if args.balance_sl_weight:
    #     loss = sum_loss + sum_loss_nls + sum_loss_sl * 10**(torch.floor(torch.log10(sum_loss.detach())))
    # else:
    #     loss = sum_loss + sum_loss_nls + sum_loss_sl
        ...
        wandb.log(
            {
                ...
                "infsb_auc_sl": true_infsb / infsb_num * 100 if infsb_num!= 0 else 0,
                "infsb_num_sl": infsb_num,
                "fsb_auc_sl": true_fsb / fsb_num * 100 if fsb_num!= 0 else 0,
                "fsb_num_sl": fsb_num,
    ...
    return  true_infsb, infsb_num , true_fsb, fsb_num

def infer_instance(model, dual_decoder,lazy_model, pyg_data, tw, distances, positions, n_ants, tw_normalize, generate_simulated_mask, normalize_phe, gen_sl_mask):
    model.eval()
    heu_vec = model(pyg_data, tw_normalize, return_embedding=gen_sl_mask)
    if gen_sl_mask:
        heu_vec, embedding = heu_vec
        if not generate_simulated_mask: # use the embedding of lazy model
            lazy_model.eval()
            with torch.no_grad():
                _, embedding = lazy_model(pyg_data, tw_normalize, return_embedding=True)
    heu_mat = model.reshape(pyg_data, heu_vec) + EPS
...
def train_epoch(
    ...
    for i in tqdm(range(steps_per_epoch), desc="Train"):
        ...
        true_infsb_batch, infsb_num_batch , true_fsb_batch, fsb_num_batch = train_instance(net,  dual_decoder, lazy_model, optimizer, data, n_ants, generate_simulated_mask,normalize_phe, cost_w,
                       invtemp, guided_exploration, shared_energy_norm, beta, it, is_train_dual_decoder, batch_size)
        if is_train_dual_decoder:
            true_infsb += true_infsb_batch
            infsb_num += infsb_num_batch
            true_fsb += true_fsb_batch
            fsb_num += fsb_num_batch
            accuracy = (true_fsb_batch + true_infsb_batch) / (infsb_num_batch + fsb_num_batch) * 100
            infsb_accuracy = (true_infsb_batch) / (infsb_num_batch) * 100
            fsb_accuracy = (true_fsb_batch) / (fsb_num_batch) * 100
            if epoch in args.train_sl_epoch_list and args.piggy_save == "batch":
                args.accuracy_isbsf = True if accuracy > args.accuracy_bsf else False
                args.fsb_accuracy_isbsf = True if fsb_accuracy > args.fsb_accuracy_bsf else False
                args.infsb_accuracy_isbsf = True if infsb_accuracy > args.infsb_accuracy_bsf else False

                args.accuracy_bsf = accuracy if accuracy > args.accuracy_bsf else args.accuracy_bsf
                args.fsb_accuracy_bsf = fsb_accuracy if fsb_accuracy > args.fsb_accuracy_bsf else args.fsb_accuracy_bsf
                args.infsb_accuracy_bsf = infsb_accuracy if infsb_accuracy > args.infsb_accuracy_bsf else args.infsb_accuracy_bsf

                if args.accuracy_isbsf:
                    # if not os.path.exists('{}/accuracy_bsf.pt'.format(savepath)) or (infsb_accuracy > 75. and fsb_accuracy > 75.):
                    print("Saving BSF accuracy ({:.4f}%) trained_model [Accuracy: {:.4f}%, Infeasible: {:.4f}%, Feasible: {:.4f}%]".format(
                            args.accuracy_bsf, accuracy, infsb_accuracy, fsb_accuracy))
                    checkpoint_dict = {
                        'epoch': epoch,
                        'dual_decoder': dual_decoder.state_dict(),
                        'model_state_dict': net.state_dict(),
                        'accuracy': accuracy,
                        'fsb_accuracy': fsb_accuracy,
                        'infsb_accuracy': infsb_accuracy,
                    }
                    torch.save(checkpoint_dict, '{}/accuracy_bsf.pt'.format(savepath))
                if args.fsb_accuracy_isbsf:
                    # if not os.path.exists('{}/fsb_accuracy_bsf.pt'.format(savepath)) or (infsb_accuracy > 75.) or (infsb_accuracy > 60. and args.problem == "TSPDL"):
                    print("Saving BSF Feasible accuracy ({:.4f}%) trained_model [Accuracy: {:.4f}%, Infeasible: {:.4f}%, Feasible: {:.4f}%]".format(
                            args.fsb_accuracy_bsf, accuracy, infsb_accuracy, fsb_accuracy))
                    checkpoint_dict = {
                        'epoch': epoch,
                        'dual_decoder': dual_decoder.state_dict(),
                        'model_state_dict': net.state_dict(),
                    ...
                    torch.save(checkpoint_dict, '{}/fsb_accuracy_bsf.pt'.format(savepath))
                if args.infsb_accuracy_isbsf:
                    # if not os.path.exists('{}/infsb_accuracy_bsf.pt'.format(args.save_dir)) or (fsb_accuracy > 75.) or (
                    #         fsb_accuracy > 60. and args.problem == "TSPDL"):
                    print("Saving BSF Infeasible accuracy ({:.4f}%) trained_model [Accuracy: {:.4f}%, Infeasible: {:.4f}%, Feasible: {:.4f}%]".format(
                            args.infsb_accuracy_bsf, accuracy, infsb_accuracy, fsb_accuracy))
                    checkpoint_dict = {
                        'epoch': epoch,
                        'dual_decoder': dual_decoder.state_dict(),
                        'model_state_dict': net.state_dict(),
                    ...
                    torch.save(checkpoint_dict, '{}/infsb_accuracy_bsf.pt'.format(savepath))

    return true_infsb, infsb_num , true_fsb, fsb_num

@torch.no_grad()
def validation(val_list, n_ants, net, dual_decoder, lazy_model, epoch, steps_per_epoch, generate_simulated_mask, normalize_phe, gen_sl_mask):
    stats = []
    outs = []
    print(len(val_list))
    for data, tw, distances, positions in tqdm(val_list, desc="Val"): # one by one
        stat, out = infer_instance(net, dual_decoder,lazy_model, data, tw, distances, positions, n_ants, tw_normalize=tw_normalize,
        ...
        wandb.log(
            {
                ...
                "val_epoch": epoch,
                "val_infsb_auc": true_infsb / infsb_num * 100 if infsb_num!=0. else 0.,
                "val_infsb_num": infsb_num,
                "val_fsb_auc": true_fsb / fsb_num * 100 if fsb_num!=0. else 0.,
                "val_fsb_num": fsb_num,
            },
            step=epoch * steps_per_epoch,
        )
...
def train(
    ...
    if args.dual_decoder:
        ...
        lazy_model.load_state_dict(net.state_dict())
        lazy_model.eval()
    ...
    if args.dual_decoder:
        ...
        optimizer = [optimizer_net, optimizer_dual_decoder]
        scheduler = [torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_net, epochs, eta_min=lr * 0.1),
                     torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_dual_decoder, epochs, eta_min=lr * 0.1)]
    else:
    ...
    best_result = validation(val_list, n_val_ants, net, dual_decoder, lazy_model, 0, steps_per_epoch, generate_simulated_mask, normalize_phe, gen_sl_mask=args.dual_decoder)
    ...
    for epoch in range(1, epochs + 1):
        ...
        start = time.time()
        true_infsb, infsb_num , true_fsb, fsb_num = train_epoch(
            args,
            n_nodes,
            tw_type,
            tw_duration,
        ...
        if epoch in args.train_sl_epoch_list:
            accuracy = (true_fsb + true_infsb) / (infsb_num + fsb_num) * 100
            infsb_accuracy = (true_infsb) / (infsb_num) * 100
            fsb_accuracy = (true_fsb) / (fsb_num) * 100
            print(">> Accuracy: {}%, Infsb_accuracy: {}%, Fsb_accuracy: {}%".format(accuracy, infsb_accuracy, fsb_accuracy))

            if epoch in args.train_sl_epoch_list and args.piggy_save == "epoch":
                args.accuracy_isbsf = True if accuracy > args.accuracy_bsf else False
                args.fsb_accuracy_isbsf = True if fsb_accuracy > args.fsb_accuracy_bsf else False
                args.infsb_accuracy_isbsf = True if infsb_accuracy > args.infsb_accuracy_bsf else False
        ...
        if epoch % val_interval == 0:
            ...
            curr_result = validation(val_list, n_val_ants, net, dual_decoder, lazy_model, epoch, steps_per_epoch, generate_simulated_mask, normalize_phe, gen_sl_mask=args.dual_decoder)
        ...
        if isinstance(scheduler, list):
            scheduler_net, scheduler_dual_decoder = scheduler
            scheduler_net.step()
            if epoch in args.train_sl_epoch_list:
                scheduler_dual_decoder.step()
        else:
            scheduler.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nodes", metavar='N', type=int, default=500, help="Problem scale")
    ...
    parser.add_argument("--val_interval", type=int, default=10, help="The interval to validate model")
    ### Logging
    ...
    parser.add_argument('--dual_decoder_type', type=str, default="AR", choices=['NAR', 'AR'])
    ...
    parser.add_argument('--balance_sl_weight', type=bool, default=True)
